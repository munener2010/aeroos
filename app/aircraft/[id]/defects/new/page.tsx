...
export default function NewDefectPage() {
  ...
  const [preventsDispatch, setPreventsDispatch] = useState(false);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        severity,
        prevents_dispatch: preventsDispatch,
      });
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-12 text-white">
      <div className="mx-auto max-w-2xl">
        ...
        <h1 className="mt-2 text-3xl font-bold">
          Report aircraft defect
        </h1>

        <p className="mt-2 text-slate-400">
          Record the issue so the appropriate team can review it.
        </p>
        ...
        >
          ...
          <label className="flex items-start gap-3 rounded-xl border border-slate-800 bg-slate-950 p-4">
            <input
              type="checkbox"
              checked={preventsDispatch}
              onChange={(event) =>
                setPreventsDispatch(event.target.checked)
              }
            ...
            <span>
              ...
              <span className="mt-1 block text-sm leading-5 text-slate-400">
                Mark this defect as requiring attention before the aircraft can
                be dispatched.
              </span>
            </span>
          </label>
